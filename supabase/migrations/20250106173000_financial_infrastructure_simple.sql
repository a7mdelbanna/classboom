...
CREATE INDEX IF NOT EXISTS idx_payment_accounts_currency ON public.payment_accounts(currency);
CREATE INDEX IF NOT EXISTS idx_exchange_rates_lookup ON public.exchange_rates(base_currency, target_currency, rate_date DESC);
CREATE INDEX IF NOT EXISTS idx_school_currency_settings_school ON public.school_currency_settings(school_id);
...
CREATE OR REPLACE FUNCTION public.increment_invoice_number(p_school_id UUID)
RETURNS VOID
LANGUAGE plpgsql
AS $$
BEGIN
